...
def _dataframe_to_exports(df: pd.DataFrame) -> ExportBundle:
    ...
    try:
        md_s = df.to_markdown(index=False)
    except Exception:  # noqa: BLE001 — tabulate may be unavailable
        md_s = f"```csv\n{csv_s}\n```"
    try:
        as_json_list = json.loads(df.to_json(orient="records", default_handler=str))
    except Exception:
    ...
    for flavor, extractor_label in (("lattice", "camelot_lattice"), ("stream", "camelot_stream")):
        try:
            with warnings.catch_warnings():
                warnings.simplefilter("ignore")
                with redirect_stderr(io.StringIO()):
                    cand = camelot.read_pdf(pdf_path, pages="all", flavor=flavor)  # type: ignore[operator]
            for tbl_idx in range(len(cand)):
                t = cand[tbl_idx]
                df = getattr(t, "df", None)
                if df is None or df.empty:
                ...
                hasher = hashlib.sha256()
                for _, row in df.iterrows():
                    txt = "|".join(row.astype(str).tolist()).encode(
                        "utf-8",
                        errors="replace",
                    )
                    hasher.update(txt)
                ...
                parsing_report = getattr(t, "parsing_report", {})
                report_val = (
                    parsing_report if isinstance(parsing_report, dict) else str(parsing_report)
                )

                out.append(
                    TableBlock(
                        table_id=(
                            f"camelot-{extractor_label}-p{page_idx}-{tbl_idx}-{uuid.uuid4().hex[:6]}"
                        ),
                        page_index=page_idx,
                        bbox=bbox_obj,
                        extractor=extractor_label,  # type: ignore[arg-type]
...
def extract_tables_pymupdf(pdf_bytes: bytes, notes: list[str]) -> list[TableBlock]:
    ...
    try:
        for pi in range(len(doc)):
            ...
            tables = getattr(table_finder, "tables", None)
            if not tables:
                continue
